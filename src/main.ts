...
async function run(): Promise<void> {
  try {
    ...
    core.debug(`set output: local version: ${local_version}`)
    ...
    core.debug(`set output: npm version: ${pkg_npm_version}`)
    core.setOutput('pkg_npm_version', `${pkg_npm_version}`)

    core.debug(`set output: Is npm version greater than local?: ${npm_is_greater}`)
